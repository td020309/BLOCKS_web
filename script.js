...
function renderCalendar() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        th.addEventListener('click', (e) => {
            ...
            if (!e.target.classList.contains('week-special-block') &&
                !e.target.classList.contains('week-special-block-more') &&
                !e.target.classList.contains('date-num') &&
                !e.target.closest('.week-special-blocks') &&
                !e.target.closest('.date-num')) {
                e.stopPropagation();
                openSpecialModal(dateStr);
                // 입력 필드로 포커스 이동
                setTimeout(() => {
